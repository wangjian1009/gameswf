...
//		ch->on_event(event_id::CONSTRUCT);	// isn't tested
	...
	bool sprite_instance::hit_test(character* ch)
	{
		...
		rect r;		// intersection
		r.m_x_min = swf_fmax(this_bound.m_x_min, ch_bound.m_x_min);
		r.m_y_min = swf_fmax(this_bound.m_y_min, ch_bound.m_y_min);
		r.m_x_max = swf_fmin(this_bound.m_x_max, ch_bound.m_x_max);
		r.m_y_max = swf_fmin(this_bound.m_y_max, ch_bound.m_y_max);
		r.twips_to_pixels();
