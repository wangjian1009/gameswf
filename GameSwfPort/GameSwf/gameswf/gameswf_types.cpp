...
//	}
	...
	// errors in tesselation.
	{
		...
		float	max_length2 = swf_fmax(basis0_length2, basis1_length2);
...
//		IF_DEBUG(log_msg("rect::read() nbits = %d\n", nbits));
	...
	{
		m_x_min = swf_fmin(m_x_min, x);
		m_y_min = swf_fmin(m_y_min, y);
		m_x_max = swf_fmax(m_x_max, x);
		m_y_max = swf_fmax(m_y_max, y);
	}
